...
Page({
  ...
  show_content:function(e){
    var that=this;
    var list_ = this.data.record_list;
    // list_=list_.map((i,v)=>{
    //   return i.show=false
    // });
    var index_ = e.currentTarget.dataset.index;
    if (list_[index_].show){
      list_[e.currentTarget.dataset.index].show = false;
      this.setData({
        record_list: list_
      })
      ...
      wx.request({
        url: `${http}/home/vip/team_data`, // 仅为示例，并非真实的接口地址
        data: {
        ...
        success(res) {
          ...
          wx.hideLoading();
          if (res.data.code == 1) {
            list_[e.currentTarget.dataset.index].show = true;
            that.setData({
              record_list: list_,
              item_detail:res.data.date
            });
          }
        }
      })
  ...
  get_record: function () {
    ...
    wx.request({
      url: `${http}/home/vip/team_list`, // 仅为示例，并非真实的接口地址
      data: {
