...
Page({
  ...
  subF: function(e) {
    ...
    formData.item_id=this.data.id;
    wx.showLoading({
      title: '添加中',
    })
    wx.request({
      url: `${http}/home/item/edit_item`, // 仅为示例，并非真实的接口地址
      data: formData,
      method: 'POST',
      header: {
  ...
  onLoad: function(options) {
    ...
    wx.request({
      ...
      success(res) {
        ...
        if (res.data.code == 1) {
              that.setData({
                img_list1: res.data.date.img,
                img_list2: res.data.date.info_img,
                goods_name: res.data.date.title,
                goods_price: res.data.date.price,
                goods_info: res.data.date.info
              })
        }
      }
    })
