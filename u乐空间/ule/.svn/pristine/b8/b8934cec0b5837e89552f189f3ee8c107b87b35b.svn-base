...
Page({
  ...
  onShow: function(options) {
    ...
    commen.updateAndGetUserInfo((user) => {
      ...
      wxReq.get(API.onlineEquipments(), { userId: user.userId }).then(resp => {
        let _equipments=resp.data.data;
  ...
  control:function(e){
    console.log(e)
    let _status=e.detail.value;
    let _id=e.currentTarget.dataset.id;
    let _command = _status?1:0;
    let _msg = _status?"设备已启动":"设备已关闭";
    wxReq.get(API.remoteController(), { command: _command,id:_id }).then(resp => {
      commen.showMessage(_msg);
    });
  },
  ...
  reviewConfirm(){
    ...
    let _userId = this.data.user.userId;
    wx.navigateTo({
      url: '../ucenter/code/info?orderId=' + _orderNo + '&id=' + _userId,
    })
  }
})